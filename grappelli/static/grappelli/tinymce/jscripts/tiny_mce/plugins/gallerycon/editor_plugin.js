		...
		init : function(ed, url) {
			...
			ed.addCommand('GalleryCon', function() {
				...
				ed.windowManager.open({
					file : url + '../../../../../../../../grappelli/gallerycon',
					width : 800 + parseInt(ed.getLang('gallerycon.delta_width', 0)),
					height : 540 + parseInt(ed.getLang('gallerycon.delta_height', 0)),
					inline : 1
				}, {
					plugin_url : url
				});
			...
			// Register buttons
			ed.addButton('gallerycon', { title : 'gallerycon.desc', cmd : 'GalleryCon' });
			// Highlight the button when an image is selected
			ed.onNodeChange.add(function(ed, cm, n) {
				cm.setActive('gallerycon', n.nodeName == 'IMG');
				// TODO: only highlight the button if the image id matches /^img__(\-?\w+?)__(\w+?)__(\w+?)$/
			});
		},
